...
class ForbiddenTest extends \PHPUnit_Framework_TestCase
{
    public function testByAccess()
    {
        ...
        $response = Forbidden::handle(new Forbidden($request, new Response()));
    ...
    public function testByContentType()
    {
        ...
        $response = Forbidden::handle(new Forbidden(new Request([]), $response));

        self::assertEquals(403, $response->getStatusCode());
        self::assertEquals('text/html; charset=UTF-8', $response->getContentType());
        self::assertContains('403', $response->getBody());
    }
}
